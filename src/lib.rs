...
#[hayashi_fn]
pub fn map(width: f64, height: f64) -> HashMap<String, HayashiValue> {
...
pub fn add_layer(
    ...
    let struct_arr = data.as_any().downcast_ref::<StructArray>();

    let geom_strings = if let Some(arr) = struct_arr {
        let geom_col_name = "geometry";
        let geom_col_idx = arr.fields().iter().position(|f| f.name() == geom_col_name);
    ...
    layer_dict.insert(
        ...
        HayashiValue::Float(stroke_width),
    );
...
pub fn render(map: HashMap<String, HayashiValue>) -> Plot {
    ...
    for (i, geom) in all_geometries.iter().enumerate() {
        let (fill, stroke, stroke_width, _point_size) = layer_configs
            .get(i)
            .cloned()
...
#[cfg(test)]
mod tests {
    use super::wkt::{compute_bounds, parse_wkt};
...
fn extract_column_string(struct_arr: &StructArray, col_idx: usize) -> Result<Vec<String>, String> {
    ...
    string_array
        .iter()
        .map(|opt: Option<&str>| {
            opt.ok_or_else(|| "null value in column".to_string())
                .map(|s| s.to_string())
        })
        .collect()
}
